...
public final class ConnectionPool {
	...
	private ConnectionPool() {
		...
		this.driver = H2_PropertyManager.getInstance().getValue(
				H2_Parameter.DB_DRIVER);
		this.url = H2_PropertyManager.getInstance().getValue(
				H2_Parameter.DB_URL);
		this.user = H2_PropertyManager.getInstance().getValue(
				H2_Parameter.DB_USER);
		this.password = H2_PropertyManager.getInstance().getValue(
				H2_Parameter.DB_PASSWORD);
		this.poolSize = Integer.valueOf((H2_PropertyManager.getInstance()
				.getValue(H2_Parameter.DB_POLL_SIZE)));
	...
	public void initPoolData() {
		allConnections = new ArrayBlockingQueue<PooledConnection>(poolSize);
		freeConnections = new ArrayBlockingQueue<PooledConnection>(poolSize);
		try {
	...
	private void clearConnectionQueue() {
		try {
			closeConnectionsQueue(allConnections);
		} catch (SQLException e) {
			e.printStackTrace();
		}
	}
